...
| **Plan 採買（MP-3）** | `/app/plan/[planId]/shopping` | 依賣場動線分區（蔬果→肉→乳製品…）；勾選即時同步；「重新整理」跟隨週菜單；完成採買可寫回冰箱 |
| **家人代買** | `/shop/[token]` | 唯讀清單 + 勾選；無法改數量／刪除；適合超市內手機操作 |

**店內採買**：以分區折疊與勾選為主，不強迫填實際金額；部分覆蓋（冰箱已有 X，再買 Y）以副文案呈現。分享連結預設 7 日有效，過期顯示友善錯誤而非空白頁。
...
| 規則 | 說明 |
|---|---|
| 預設 | 效期「智慧提醒」開啟；週報預設關閉（需使用者開啟） |
| 安靜時段 | 預設 22:00–08:00（使用者時區，預設 `Asia/Taipei`）；跨午夜以 OR 判斷 |
| 勿騷擾 | `snooze`、關閉提醒、連續 3 次忽略 → 14 天 backoff |
| 交付 | App 收件匣橫幅（非 LINE）；每則提醒必須一鍵「暫停／設定」 |
| 首次 | 第一則提醒前顯示免責說明與關閉方式 |
| 隱私 | 日誌／指標不記錄食材名稱；`user_id` 僅 hash 後上報 |

DST：台灣無夏令時間；仍使用 IANA 時區計算本地小時。

## 5b) 菜單執行期（MP-4，Web）

| 規則 | 說明 |
|---|---|
| 推播時機 | 每日預告（預設 09:00）、晚餐前（預設 17:00）、採買日、週末回顧；寫入收件匣 |
| 語氣 | 鼓勵而非碎念；跳過不羞辱（「實際生活總有變數」） |
| 我做了 | 先標記 cooked，再顯示冰箱扣減確認；「適量」預設不勾 |
| 安靜時段 | 若 `quiet_hours_end > 早上推播小時`，略過晨間推播（使用者等於拒絕早晨打擾） |
| 忽略 | 晨間推播連續 3 次未互動 → 14 天 backoff（與效期提醒分開計數） |
