...
| 規則 | 說明 |
|---|---|
| 等待 | 規劃約 15–30 秒：立即 ack「已開始規劃」+ 輪詢進度文案（candidate → validate → repair） |
| 取消 | 生成中可「取消」→ `abandoned`；勿讓使用者盯白屏 |
| 空冰箱 | 摘要可省略冰箱項，避免罪惡感；warnings 可提示「多數需採買」 |
| 軟門檻 | 免費每月 2 次；429 附 quota 說明，不阻塞瀏覽既有草稿 |
| 採買 | 「看採買清單」MP-2 為 placeholder（MP-3 再接聚合） |
| 換菜 | 先給 2–3 候選再套用；成功 toast 菜名即可 |
| LINE | 本產品已移除 LINE；不在此規範 Flex 流程 |
