...
import { recordMealPlanActivate } from "@/platform/observability/meal-planning-metrics";
...
export async function POST(_request: Request, { params }: Params) {
  ...
  const existing = await getMealPlan(id, auth.tenantId, auth.userId, {
    include_slots: false,
  });
  if (!existing || existing.status === "generating") {
    return planForbidden();
  }

  const plan = await activateMealPlan(id, auth.tenantId, auth.userId);
  if (!plan) return planForbidden();

  recordMealPlanActivate();
