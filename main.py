...
SYSTEM_PROMPT = (
    "米其林研發團隊。三位主廚(行政主廚、副主廚、食材總管)各簡短一句討論再產出食譜。\n"
    "kitchen_talk 每則限 30 字內。嚴格輸出以下 JSON，字串不可為空：\n"
...
async def get_filtered_user_memory(user_id: str) -> list:
    """
    撈取過濾後的對話紀錄：只保留 context_updated_at 之後的訊息。
    若使用者從未切換菜系，則回傳完整歷史。
    """
    full_history, (_, context_updated_at) = await asyncio.gather(
        get_user_memory(user_id),
        get_user_cuisine_context(user_id),
    )
    return _filter_history_after_context(full_history, context_updated_at)


async def _fetch_ai_context(user_id: str) -> tuple[list, list, str | None]:
    """
    一次查詢取得 full_history、filtered_history、active_cuisine，避免重複 DB 呼叫。
    回傳 (full_history, filtered_history, active_cuisine)。
    """
    full_history, (active_cuisine, context_updated_at) = await asyncio.gather(
        get_user_memory(user_id),
        get_user_cuisine_context(user_id),
    )
    filtered = _filter_history_after_context(full_history, context_updated_at)
    return full_history, filtered, active_cuisine
...
async def _build_system_prompt_with_preferences(user_id: str, current_cuisine: str | None = None) -> str:
    prefs = await get_user_preferences(user_id)
    base = SYSTEM_PROMPT
    if prefs:
        base += f"\n飲食禁忌：{prefs}。設計時須避開。"
    if current_cuisine and current_cuisine != "不拘":
        base += f"\n\n料理情境：{current_cuisine}。忽略無關過往討論，聚焦此風格。遵守飲食禁忌。"
    elif current_cuisine == "不拘":
        base += "\n\n未指定料理情境，可自由發揮。遵守飲食禁忌。"
    return base
...
async def process_ai_reply(event: WebhookMessageEvent) -> None:
    ...
    full_history, filtered_history, active_cuisine = await _fetch_ai_context(user_id)
    current_cuisine = CUISINE_LABELS.get(active_cuisine or "", active_cuisine or "不拘")
    effective_system = await _build_system_prompt_with_preferences(user_id, current_cuisine)
