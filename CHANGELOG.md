...
- **MP-2 週菜單規劃 UI**：`GET|POST /api/me/meal-plans`、`GET /api/me/meal-plans/[id]`、`POST …/activate`；頁面 `/app/plan/generate`（觸發 AI 規劃）、`/app/plan/sessions/[id]`（檢視 slots、啟用草稿）；週菜單／儀表板入口。
...
- **階段 8 驗證**：`pnpm lint` ✅、`pnpm test` ✅、`pnpm build` ✅（shared First Load JS **104 kB**；最重頁 `/app/pantry/scan` **158 kB**、 `/app/plan` **171 kB**）；Playwright **4/4** ✅（本機需 `pnpm exec playwright install chromium`）。
